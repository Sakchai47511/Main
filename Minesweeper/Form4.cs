...
namespace ProjectProgram
{
    public partial class Form4 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            cmd.CommandText = $"SELECT * FROM score01 WHERE Name=\"{textBox1.Text}\"";
            MySqlDataReader row = cmd.ExecuteReader();
            if (row.HasRows)
            {
                while (row.Read())
                {
                    if (int.Parse(row.GetString(Program.check)) > Program.time_form||row.GetString(Program.check)=="")
                    {
        ...
        //con.Open();
        //cmd.CommandText = $"INSERT INTO score01(Name,Time) VALUES (\"{textBox1.Text}\",@time)";
